...
class Absorbing(Graph):
    ...
    def staggered_score(self, score, dsigma):
        out_score = score.clone() # yeah yeah whatever we should probably do this
        extra_const = (1 - (dsigma).exp()) * score.sum(dim=-1)
        # import pdb; pdb.set_trace();
        out_score = out_score * dsigma.exp()[:, None]
        # mask = torch.zeros_like(score)
        # mask[..., -1] = 1
        # # score.masked_fill_(mask.bool(), extra_const)
        # score = score + mask * extra_const
        out_score[..., -1] = out_score[..., -1] + extra_const
        return score

    def sample_limit(self, *batch_dims):
        return (self.dim - 1) * torch.ones(*batch_dims, dtype=torch.int64).requires_grad_(False)
